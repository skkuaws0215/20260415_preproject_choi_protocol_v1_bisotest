...
def main() -> int:
    ...
    files_to_check = {
        ...
        "depmap_crispr_long": processed_depmap / "depmap_crispr_long_stad.parquet",
    ...
    schemas = {
        ...
        "depmap_crispr_long": ["cell_line_name", "gene_name", "dependency"],
    }
    ...
    if "labels" in dfs and "depmap_crispr_long" in dfs:
        lc = set(dfs["labels"]["sample_id"].astype(str))
        dc = set(dfs["depmap_crispr_long"]["cell_line_name"].astype(str))
        missing = lc - dc
        report["consistency"]["labels_cells_in_depmap"] = len(lc & dc)
        report["consistency"]["labels_not_in_depmap"] = len(missing)
        # Informational only (Colon: StrippedCellLineName vs Model.CellLineName mismatch is common).
        if missing:
            report["warnings"].append(
                f"{len(missing)} label sample_id not in depmap cell_line_name "
                f"(e.g. {sorted(missing)[:5]}) — verify Model.CellLineName vs StrippedCellLineName mapping."
            )
