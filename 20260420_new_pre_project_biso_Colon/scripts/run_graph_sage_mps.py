...
def train_evaluate_graphsage(X, y, groups, output_stem, oof_dir=None, k_neighbors=7):
    """
    ...
    try:
        ...
        if "out of memory" in str(e).lower() or "oom" in str(e).lower():
            ...
            # Retry on CPU
            return train_evaluate_graphsage(X, y, groups, output_stem, oof_dir, k_neighbors)
        else:
            raise e
...
if __name__ == "__main__":
    ...
    # Run GraphSAGE with Colon k=7
    results = train_evaluate_graphsage(X, y, groups, "colon_numeric_graph_v1", oof_dir, k_neighbors=7)

    # Load existing results and merge (GAT may have written first)
